ITH funnel_stages AS (
  ...
  FROM `sql-project-500400.projeto_sql.fonte`          

  -- Filtra os 30 dias anteriores à última data registrada na tabela
  WHERE event_date >= DATE_SUB((SELECT MAX(event_date) FROM `sql-project-500400.projeto_sql.fonte`), INTERVAL 30 DAY)
)

--- taxa de conversao no funil
SELECT 
  stage_1_views,
  stage_2_cart,
  ...
  ROUND(stage_5_purchase * 100 / stage_4_payment) AS overall_convertion_rate
